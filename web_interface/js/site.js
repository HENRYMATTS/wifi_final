...
var connectivityInterval = null;
var isDeviceConnected = true;

function pingDevice() {
    var xhr = new XMLHttpRequest();
    // Cache‑busting + use a lightweight endpoint (root is fine)
    xhr.open("GET", "/?_=" + Date.now(), true);
    xhr.timeout = 3000;
    xhr.onload = function() {
        if (xhr.status === 200) {
            if (!isDeviceConnected) {
                isDeviceConnected = true;
                // Do NOT auto‑set green – leave that to user actions
            }
        } else {
            setDisconnected();
        }
    };
    xhr.onerror = function() { setDisconnected(); };
    xhr.ontimeout = function() { setDisconnected(); };
    xhr.send();
}
...
function setDisconnected() {
    if (isDeviceConnected) {
        ...
        showMessage("ERROR: disconnected");   // ✅ now turns red
    }
}

// Start monitor after page load, ensure only one interval exists
window.addEventListener('load', function() {
    if (connectivityInterval) clearInterval(connectivityInterval);
    connectivityInterval = setInterval(pingDevice, 5000);
});