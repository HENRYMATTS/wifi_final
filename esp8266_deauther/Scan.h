...
struct ProbeEntry {
    ...
    uint8_t channel;
    int8_t rssi;              // placeholder – not available in this sniffer, but we keep for future
    uint32_t lastSeen;      // timestamp (millis)
    uint16_t count;         // how many times this probe was seen (aggregated)
    char vendor[33];
};
...
class Scan {
    public:
        ...
        void startProbe(uint32_t time, bool channelHop, uint8_t channel);
        void saveProbeResults();
    ...
    private:
        ...
        ProbeEntry probeEntries[MAX_PROBE_ENTRIES];
        uint8_t probeEntryCount = 0;
        bool probeScanActive = false;
        int findProbeEntry(uint8_t* mac);
        void addOrUpdateProbe(uint8_t* mac, const char* ssid, uint8_t channel, int8_t rssi, const char* vendor);
