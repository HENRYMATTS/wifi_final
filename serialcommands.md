...
i think we should follow the spacehuhn concept here so this is how we will do it  create 3 new files sniff.html sniff.js and sniff.json  to handle this feature 

the sniff will be added at the end of that line that shows scan ssid attack then sniff so that when i tap sniff 



can spacehuhn project sniff probe and becon pasckets too with the scan wifi command or it only does deauth 
...
okay this is what i want to ask can the esp8266 esp-01 spacehuhn deauther do a probe request sniff like stefan kremser the author of the spacehuhn project did not include this kind of attack that listens to phones call out the networks they ever connected to before so i wondering if we can implement it in our own way so is it possible no code lets discuss