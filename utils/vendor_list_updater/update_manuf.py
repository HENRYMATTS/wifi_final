...
WS_MANUF_FILE_URL = "https://www.wireshark.org/download/automated/data/manuf"
macs = []
vendors = []
tempVendors = []
...
def generate_files(output):
    ...
    for mac in macs:
        ...
        if len(octets) < 3:
            # Skip if the line doesn't have at least two colons
            continue
        (oc1, oc2, oc3) = octets[0:3]
        if vendorindex > 255:
            num = vendorindex
            index_bytes = []
            while num > 0:
